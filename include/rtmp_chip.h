...
#define IS_DUAL_BAND_NIC(_pAd) (((_pAd->RfIcType == RFIC_2850) || (_pAd->RfIcType == RFIC_2750) || (_pAd->RfIcType == RFIC_3052) || (_pAd->RfIcType == RFIC_3053) || (_pAd->RfIcType == RFIC_2853) || (_pAd->RfIcType == RFIC_3853) || IS_RT_NEW_DUAL_BAND_NIC(_pAd)) && !IS_RT5390(_pAd))
...
struct _RTMP_CHIP_OP_
{
    ...
    /* Power save */
    VOID(*EnableAPMIMOPS)
    (struct _RTMP_ADAPTER *pAd, IN BOOLEAN ReduceCorePower);
    VOID(*DisableAPMIMOPS)
    (struct _RTMP_ADAPTER *pAd);
    INT(*PwrSavingOP)
    (struct _RTMP_ADAPTER *pAd, UINT32 PwrOP, UINT32 PwrLevel,
     UINT32 ListenInterval, UINT32 PreTBTTLeadTime,
     UINT8 TIMByteOffset, UINT8 TIMBytePattern);

    /* Chip tuning */
    VOID(*RxSensitivityTuning)
    ...
    /* AGC */
    VOID(*ChipAGCInit)
    (struct _RTMP_ADAPTER *pAd, UCHAR bw);
    UCHAR(*ChipAGCAdjust)
    (struct _RTMP_ADAPTER *pAd, CHAR Rssi, UCHAR OrigR66Value);

    /* Channel */
    VOID(*ChipSwitchChannel)
    (struct _RTMP_ADAPTER *pAd, UCHAR ch, enum SWITCH_CHANNEL_STAGE Stage);
    ...
    (struct _RTMP_ADAPTER *pAd, ULONG *TxPwr);
    ...
    VOID(*SetRxAnt)
    (struct _RTMP_ADAPTER *pAd, UCHAR Ant);
    ...
    (struct _RTMP_ADAPTER *pAd, ULONG threshold);
#endif /* CARRIER_DETECTION_SUPPORT */
    ...
    VOID(*Calibration)
    (struct _RTMP_ADAPTER *pAd, UINT32 CalibrationID, UINT32 Parameter);
    VOID(*SecondCCADetection)
    (struct _RTMP_ADAPTER *pAd);
